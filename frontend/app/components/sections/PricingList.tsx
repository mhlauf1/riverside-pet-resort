          ...
          <div
            className={`mx-auto ${
              isSingleCard
                ? 'max-w-md'
                : isTwoCol
                  ? 'grid max-w-4xl grid-cols-1 gap-4 sm:grid-cols-2'
                  : 'max-w-4xl'
            }`}
          >
            {items.map((item, i) => (
              <FadeIn key={item._key} delay={0.03 * i}>
                {isTwoCol || isSingleCard ? (
                  <div className="h-full rounded-md border border-border-light bg-white/70 p-6 text-center shadow-sm">
                    {item.service && (
                      <h3 className="font-sans text-[16px] md:text-[18px] font-semibold text-forest">
                        {item.service}
                      </h3>
                    )}
                    {item.price && (
                      <p className="mt-3 text-[28px] md:text-[40px] font-semibold leading-none tracking-tight text-terracotta">
                        {item.price}
                      </p>
                    )}
                    {item.note && (
